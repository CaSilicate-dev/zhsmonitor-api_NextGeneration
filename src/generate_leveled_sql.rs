...
pub fn gl_server_stat(table: &str, level: i32) -> Result<String, SqlGeneratorError> {
    ...
    Ok(generate_sql::generate_server_stat(
        table,
        ranges.to_string().as_str(),
        bs.to_string().as_str(),
    ))
}
...
pub fn gl_mc_stat(table: &str, level: i32) -> Result<String, SqlGeneratorError> {
    let (ranges, bs) = generate_drs(level)?;
    Ok(generate_sql::generate_mc_stat(
        table,
        ranges.to_string().as_str(),
        bs.to_string().as_str(),
    ))
...
pub fn gl_current_general(
    ...
    Ok(generate_sql::generate_current_general(
        compression_type,
        data_type,
        table,
        ranges,
